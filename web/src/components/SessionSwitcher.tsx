...
type DrawerTab = "active" | "archived" | "all";

const TAB_ARCHIVED: Record<DrawerTab, "false" | "true" | "all"> = {
  active: "false",
  archived: "true",
  all: "all",
};

const DRAWER_TAB_KEY = "session_drawer_tab";

function readStoredTab(): DrawerTab {
  const v = localStorage.getItem(DRAWER_TAB_KEY);
...
export function SessionSwitcher({
  ...
  function changeTab(next: DrawerTab) {
    setTab(next);
    localStorage.setItem(DRAWER_TAB_KEY, next);
  }
  ...
  const headerLabel =
    currentTitle ||
    (currentSessionId ? "未命名会话" : "新对话");

  return (
    <>
      <button
        type="button"
        className="max-w-[10rem] truncate rounded border border-slate-700 bg-slate-900/80 px-2 py-1 text-left text-slate-200 hover:border-emerald-700/60"
        onClick={openDrawer}
        title={headerLabel}
      >
        {headerLabel}
      ...
      {open ? (
        <div className="fixed inset-0 z-40 flex">
          ...
          />
          <aside className="flex h-full w-full max-w-sm flex-col border-l border-slate-700 bg-slate-950 shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
              <h2 className="font-medium text-slate-100">会话</h2>
              <button
                type="button"
