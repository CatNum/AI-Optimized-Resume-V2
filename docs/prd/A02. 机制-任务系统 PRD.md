...
| `active` | **进行中** | 按 [§5.8.5](./A02.%20机制-任务系统%20PRD.md#585-工具与状态机强约束) 强约束推进 | 用户在**对话**中明确表示立即开始（如「开始执行」「现在开始」；协调者解析后 `start_task_list`，**无**专用 UI 按钮） |
...
- `ready` 与 `active` 的 list **均可** 同时存在于磁盘（**同一 session 内**；例如一个 JD 计划待开始、用户先去闲聊）；仅 **`active`** 占用「当前执行」槽位并写入 `_active.json`（若启用）。
- **Session 绑定**：`create_task_list` 须写入 `meta.session_id`；`POST /v1/sessions/new`（或换会话）时 **删除** 该 `session_id` 下 **全部** list（含 `ready` + `active`）并清 `_active.json`（见 [架构 10 §1](../architecture/10-会话闸门与state.md#1-会话工作区生命周期)）。**跨会话**任务进度不保留；长期事实仍在 `profile.json`。
...
**简历优化**：用户确认里程碑「按 JD 优化简历」并 **多选档位** 后，入口编排智能体 **仅路由**——完成该 milestone 的闸门并 **指派** 给 **简历智能体**（Run 内自选 `load_skill("resume-module-optimize")`）。**如何优化、是否按模块/档位拆分为多条 `work`、逐条 claim/complete、HTML 落盘（`write_resume_html`）及 `html_deliveries` 回传** 均由 **简历智能体** 负责；协调者再派 **资产智能体** 调用 `register_outputs_index` 登记索引（**不** 写 HTML 文件）。
...
```json
{
  ...
  "session_id": "sess_abc..."           // 创建该 list 时的会话 ID；换会话时删除该 session 下全部 list
}
```
...
| `session_id` | 所属会话 | 与 `data/sessions/{session_id}/` 一致；`create_task_list` **必填** |
...
> **已废弃**：`metadata.skill_name`。Skill 由被指派的 Worker 在 Run 内 **自选** `load_skill`（见 [A03](./A03.%20机制-技能包%20PRD.md)、[架构 02 §2](../architecture/02-平台服务.md#2-skill-管理注册表--worker-自选)）。
...
| 存在 `meta.json` 且 `status = ready`（同时有 `{task_id}.json`） | 任务列表（**只读计划**）；用户在对话输入框表达「开始执行」等，**无**专用按钮 |
...
| 某 JD 全流程 | `jd` | JD 录入 → 市场/岗位分析 → 职业战略与投递策略 → 确认优化 | 简历智能体确认优化后 **自行创建** N 条 `work`（每档一行：保守→标准→进取）；HTML 落盘；资产智能体 `outputs_index` |
| 纯规划 | `plan` | 探讨落档 | — |
