...
- **Python 单体**：本机 **单进程** `career_os` 承载 API、Harness、平台服务、协调者/Worker 与 LLM 调用（**不引入 Go**）。
...
flowchart LR
  U[用户] --> Web[Web 前端]
  Web -->|REST + SSE| App[career_os Python]
  subgraph App
    API[FastAPI]
...
sequenceDiagram
  ...
  alt 需派工
    loop 无 gate_prompt 时可连派
      C->>H: execute_tool(delegate_worker)
      H->>W: worker_run（不推 SSE）
      W->>Store: 经 Harness 工具读写
      W-->>H: WorkerResult
      H-->>C: 工具结果
    end
    Note over C: 遇 gate_prompt 或本轮结束 → synthesize
    C->>U: 流式合成回复（SSE token）
  else 简单问答
    C->>U: 流式直接回复
  end
...
| [02-平台服务.md](./02-平台服务.md) | Prompt / Skill / **Worker Registry** / Tool / Task / 存储 |
...
| [09-Worker结构化输出.md](./09-Worker结构化输出.md) | 各 Worker `structured_output` Pydantic 契约 |
| [10-会话闸门与state.md](./10-会话闸门与state.md) | Session 工作区、闸门、`state.json` |
| [11-L7-浏览器Tool.md](./11-L7-浏览器Tool.md) | `browser_fetch` 混合实现与降级 |
| [12-评测与可观测.md](./12-评测与可观测.md) | Eval 分层、JSONL trace、≥20 case |
| [13-Profile-写入权限.md](./13-Profile-写入权限.md) | 可见/不可见写入、白名单 × actor |
| [14-Harness-Tools-Schema.md](./14-Harness-Tools-Schema.md) | Harness Tool JSON Schema 全集索引 |
...
- **跨会话**持久化对话与任务进度（换会话清空 session 工作区；长期事实仅 `profile.json` / `output/`，见 [10-会话闸门与state.md](./10-会话闸门与state.md)）。
