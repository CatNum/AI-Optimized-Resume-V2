...
| 最后更新 | 2026-05-30（C1 存储锁、13/14 索引） |
...
| work | （由 resume Worker 动态追加：每所选档位一条，保守→标准→进取） | 优化 milestone 完成后 |
...
- **Session 工作区**（gitignore）：
  ...
  - `state.json`：派工中间态（`prior_results`、`gates`、`list_id`、`last_activity_at` 等）。
