...
sequenceDiagram
  ...
  W-->>H: WorkerResult
  H->>C: plan_proposal / accepted_async
  C->>H: delegate_worker(opportunity)
  Note over H: 每次解析正式引用、有效期和用户确认
  H->>W: context.market_research_result
```
...
| **JD-R1** | `delegate_worker(opportunity)` 前必须解析当前正式市场引用，验证版本、有效期、删除状态和用户确认；旧 `prior_results.market` 不授权 |
...
**职责**：工具注册、权限、参数校验、审计。与 Skill 类似：**协调者派工时只给 `tool_index`（名称 + 描述 + 参数摘要）**；Worker 在 Run 内 **自行选择** 调用哪些 tool。市场工具见 [11-市场调研Tool.md](./11-市场调研Tool.md)。
...
| `market_tools` | market | 仅 `market_research(plan_id)`；不能提交关键词、城市、action 或 URL |
...
- 正式市场结果仍在运行、未确认、过期、删除或版本不一致 → 市场阶段之后的 Worker 拒绝（见 [§3.3](#33-派工模式v01)）
...
**Preset: `pipeline`（`list_type=pipeline`，2026-06）**
...
| resume_optimize | 简历优化（须 optimize_confirm 后 advance，不可 jump 直达） |

模板 SSOT：`config/pipeline_milestones.json`；`POST /v1/sessions/new` 实例化。
