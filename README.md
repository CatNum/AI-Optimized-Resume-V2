...
- 当前产品文档版本：`v2.1（开发中）`，用于说明当前交付范围与状态；它不同于代码包版本。
- 分支策略：只维护 `main`。
...
当前产品版本是 [v2.1（开发中）](docs/roadmap/v2.1.md)，只能确认以下边界：
...
[v2.2（规划中）](docs/roadmap/v2.2.md)只确认以下顺序，不自动吸收长期候选项：
...
3. **最后执行跨模块系统级回归。** 在干净临时环境证明上游失败会阻断下游、部分成功会保留真实成果、运行身份与 Trace 可以关联。
