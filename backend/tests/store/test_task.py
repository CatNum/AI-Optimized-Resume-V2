...
def test_create_second_active_same_session_returns_error(task_store):
    assert isinstance(
        task_store.create_task_list("sess_a", list_type="jd", status="active"), str
    )
...
def test_normalize_multi_active_keeps_newest(task_store, tmp_path, caplog):
    id_old = task_store.create_task_list("sess_a", list_type="explore", status="active")
    assert isinstance(id_old, str)
    id_new = task_store.create_task_list("sess_a", list_type="jd", status="ready")
...
def test_list_lists_for_session_orders_active_then_ready(task_store):
    active = task_store.create_task_list("sess_a", list_type="explore", status="active")
    ready = task_store.create_task_list("sess_a", list_type="jd", status="ready")
    rows = task_store.list_lists_for_session("sess_a")
    assert len(rows) == 2
...
def test_list_lists_for_session_ready_sorted_by_updated_at(task_store, tmp_path):
    ...
    newer = task_store.create_task_list("sess_a", list_type="jd", status="ready")
...
def test_list_lists_for_session_filters_other_sessions(task_store):
    task_store.create_task_list("sess_a", list_type="explore", status="active")
    task_store.create_task_list("sess_b", list_type="explore", status="active")
    rows = task_store.list_lists_for_session("sess_a")
    assert len(rows) == 1


def test_delete_lists_for_session(task_store, tmp_path):
    list_id = task_store.create_task_list("sess_a", list_type="jd")
    task_store.create_task(list_id, "milestone_1", "Step")
    other_id = task_store.create_task_list("sess_b", list_type="jd")
